...
class sync_same_table:
    ...
    def _get_id(self):
        col = pd.read_sql("SELECT column_name FROM INFORMATION_SCHEMA.`KEY_COLUMN_USAGE`"
                          "WHERE table_name='{}' "
                          "AND CONSTRAINT_SCHEMA='data_test'"
    ...
    def generate_condition(self,dataframe):
        ...
        for i in range(len(del_target)):
            if i==0:
                condition.append("{} = '{}' ".format(self.target_index[0],del_target[0]))
            else:
                condition.append("and {} = '{}' ".format(self.target_index[i], del_target[i]))
...
if __name__=='__main__':
    ENGINE_RD = create_engine(
        "mysql+pymysql://{}:{}@{}:{}/{}".format('jr_read_17', 'jr_read_17', '182.254.128.241', 4171, 'base', ),
    ...
    gg=sync_same_table('fund_nv_data_standard_copy3',engine_data_test,'fund_nv_data_standard_copy3_copy',engine_data_test,chunck_size=10)
    gg.sync()
    print('finish')