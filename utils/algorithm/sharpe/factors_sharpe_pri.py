...
class Const:
    ...
    __s = [
        ...
        "6010201",  # 管理期货-期货趋势,
        "6010202",  # 管理期货-期货套利,
        "6010203",  # 管理期货-其他管理期货策略,
        "6010301",  # 相对价值-ETF套利,
        "6010302",  # 相对价值-可转债套利,
        "6010303",  # 相对价值-固定收益套利,
        "6010304",  # 相对价值-分级基金套利,
        "6010305",  # 相对价值-其他相对价值策略,
        "6010306",  # 相对价值-期权套利,
        "6010601",  # 宏观策略-宏观策略,
        "6010801",  # 多策略-多策略,
    ]
    ls = {**{}.fromkeys(__l, "l"), **{}.fromkeys(__s, "s")}
...
class StyleConstraints(BaseConstraints, LsConstraints):
    ...
    def _init_cons_by_stype(self):
        # 股票型 {策略(除市场中性), 事件驱动}
        if self.stype in {"6010101", "6010102", "6010401", "6010402", "6010403", "6010404"}:
            self._cons.extend([
                {"type": "ineq", "fun": lambda x: sum(x[self.tf[FactorClassification.STOCK_CODE]]) - 0.4},
                {"type": "ineq", "fun": lambda x: sum(x[self.tf[FactorClassification.CASH_CODE]]) - 0},
                # 股票类型因子权重大于等于0.4
            ])

        # 债券策略
        elif self.stype in {"6010501"}:
            self._cons.extend([
                {"type": "ineq", "fun": lambda x: sum(x[self.tf[FactorClassification.BOND_CODE]]) - 0.6},
                {"type": "ineq", "fun": lambda x: sum(x[self.tf[FactorClassification.CASH_CODE]]) - 0},
            ])

        # 混合策略
        elif self.stype in {
            "6010301", "6010302", "6010303", "6010304", "6010305", "6010306",  # 相对价值
            "6010601", "6010701", "6010702", "6010703", "6010704", "6010801",  # 宏观, 组合, 多策略
        }:
            tf = self.tf[FactorClassification.STOCK_CODE] | self.tf[FactorClassification.BOND_CODE] | self.tf[FactorClassification.NHF_CODE]
            self._cons.extend([
                # 非现金因子权重大于等于0.4
                {"type": "ineq", "fun": lambda x: sum(x[tf]) - 0.4},
                {"type": "ineq", "fun": lambda x: sum(x[self.tf[FactorClassification.CASH_CODE]]) - 0},
            ])

        # 期货型
        elif self.stype in {"6010201", "6010202", "6010203"}:
            self._cons.append(
                {"type": "ineq", "fun": lambda x: sum(x[self.tf[FactorClassification.CASH_CODE]]) - 0},
            )
    ...
    def initialize(self):
        if not self.initialized:
            self._init_bnds_by_ls()  # 初始化边界约束
            self._cons_by_ls()  # 初始化多空约束
...
# 私募基金接口调用
class PriSharpeFactor(SharpeFactor):
    def __init__(self, fund_id: str, start: dt.date, end: dt.date, freq: str, factor_type: str, ftypes=None, **kwargs):
        ...
        if ftypes is None:
            ...
            if self.fund.stype in {"6010101", "6010102", "6010103", "6010401", "6010402", "6010403", "6010404"}:
                if self.factor_type == "style":
            ...
            # 债券策略
            elif self.fund.stype in {"6010501"}:
                if self.factor_type == "style":
            ...
            # 期货型
            elif self.fund.stype in {"6010201", "6010202", "6010203"}:
                if self.factor_type == "style":
                    ftypes = FactorUsed.FUTURE
...
def main():
    ...
    fid, s, e, freq, factor_type = "JR000001", None, dt.date(2018, 5, 28), "w", "style"
    res = PriSharpeFactor(fid, s, e, freq, factor_type, tol=1e-18, options={"maxiter": 1000})
    pprint(res)
